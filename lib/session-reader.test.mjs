...
test("defers historical thinking without changing live-session content", () => {
  const entries = [
    userEntry("u1", null, "start"),
    {
      ...
      message: {
        ...
        model: "test-model",
        content: [
          { type: "thinking", thinking: "**Checking declarations**\n\nlarge reasoning" },
  ...
  assert.deepEqual(deferred.messages[1].content[0], {
    ...
    thinking: "",
    deferred: true,
    summary: "Checking declarations",
  });

  const full = buildSessionContext(entries);
  assert.equal(full.messages[1].content[0].thinking, "**Checking declarations**\n\nlarge reasoning");
});
...
test("defers only base64 images from historical tool results", () => {
  ...
  assert.deepEqual(deferred.messages[2].content[1], { type: "image", deferredImage: { entryId: "tr1", blockIndex: 1 } });
  assert.deepEqual(deferred.messages[2].content[2], { type: "image", deferredImage: { entryId: "tr1", blockIndex: 2 } });
