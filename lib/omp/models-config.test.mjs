...
const { REDACTED_MODEL_CONFIG_VALUE } = await jiti.import("../models-config-drafts.ts");
...
test("redacts API keys and headers without deleting them on save", () => {
  withAgentDir((dir, path) => {
    mkdirSync(dir, { recursive: true });
    writeFileSync(path, HAND_EDITED, "utf8");
    const current = readModelsConfigFile().config;
    current.providers["work-proxy"].headers = { Authorization: "Bearer secret" };
    writeModelsConfig(current);

    const persisted = readModelsConfigFile().config;
    const client = redactModelsConfig(persisted);
    assert.equal(client.providers["work-proxy"].apiKey, REDACTED_MODEL_CONFIG_VALUE);
    assert.equal(client.providers["work-proxy"].headers.Authorization, REDACTED_MODEL_CONFIG_VALUE);

    client.providers["work-proxy"].models[0].reasoning = false;
    writeModelsConfig(restoreRedactedModelsConfig(client, persisted));
    const restored = readModelsConfigFile().config.providers["work-proxy"];
    assert.equal(restored.apiKey, "!op read op://work/openai/key");
    assert.equal(restored.headers.Authorization, "Bearer secret");
    assert.equal(restored.models[0].reasoning, false);
  });
});
