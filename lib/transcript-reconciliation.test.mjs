...
test("confirmation replaces optimistic identity before a feed replay", async () => {
  const { appendDeliveredUserMessage, userMessageKey } = await loadSubject();
  const optimistic = user("Here", 10);
  const confirmed = user("Here", 11);
  const first = appendDeliveredUserMessage([optimistic], confirmed, userMessageKey(optimistic));
  assert.deepEqual(first, [confirmed]);
  assert.strictEqual(appendDeliveredUserMessage(first, confirmed, null), first);
  const repeatedOnPurpose = user("Here", 12);
  assert.deepEqual(appendDeliveredUserMessage(first, repeatedOnPurpose, null), [confirmed, repeatedOnPurpose]);
});

test("interrupted run output between optimistic prompt and delivery does not duplicate the prompt", async () => {
  const { appendDeliveredUserMessage, userMessageKey } = await loadSubject();
  const optimistic = user("Here", 10);
  const confirmed = user("Here", 11);
  const trailing = assistant("Interrupted output", 12);
  const result = appendDeliveredUserMessage([optimistic, trailing], confirmed, userMessageKey(optimistic));
  assert.deepEqual(result, [confirmed, trailing]);
  assert.strictEqual(appendDeliveredUserMessage(result, confirmed, null), result);
});
