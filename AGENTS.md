...
Tasks and Subagents live in the shared top bar in place of the redundant thread label, opening independent downward popovers without resizing the composer or timeline. The task trigger previews the active task (blocked fallback), with counts secondary. The feed ends with a stable 16px activity row: latest thinking text while busy, Idle otherwise, retaining the server-owned kaomoji. Only busy expressions animate; reduced motion disables animation. Empty-expression reminders explicitly ask for a kaomoji and caption; persisted older reminders still strip correctly. Consecutive user messages omit intermediate timestamp/action rows; metadata remains available on hover/focus without reflow. Async-result logs render as literal preformatted text, not Markdown.
Activity scans the current user turn backwards through tool results, preferring the latest double-star thinking summary without literal Markdown delimiters. Deferred local thinking retains a bounded summary. Compact transcript mode omits repeated Thinking rows; full process mode retains reasoning access. Activity typography shares the conversation font and size token. Text streaming counts as busy; busy kaomoji use a visible 0.9s bounce/scale/opacity cycle, with idle and reduced-motion animation disabled. Tool rows use action icons plus file-type icons for paths and the latest available result line while collapsed.
Tool expansion shows results, not a duplicate command header. Bash uses a plain monospace dollar prompt and borderless highlighted command. File reads preserve returned line information with syntax highlighting; successful writes show submitted contents plus acknowledgment, edits retain diffs. Grep/glob recognize explicit grouped/file-list formats and otherwise preserve raw output. Eval shows source and actual output; web result URLs are safe external links; task summaries retain per-agent outcomes and hub sends show message contents. No counts or output are fabricated when absent.
Transcript and composer share an 840px column centered within the area excluding the 36px desktop minimap rail; empty-state composer avoids nested gutters. Chat prose caps at 78ch while code/tables keep column width. Dormant goals, task plans, and terminal subagent history remain accessible through the header’s Tasks and history dialog rather than occupying the composer. Mobile left-edge right swipes open the existing workspace drawer; vertical and non-edge gestures do not. Streaming emphasis is a display-only remark transform on final-paragraph text; completed Markdown and code retain normal parsing.
...
Task previews show unfinished work first; completed/abandoned tasks follow on disclosure. Add task uses native `/todo append` to mutate the current plan atomically without an agent turn or interruption. SVG SmoothSurface backgrounds provide composer and user-bubble contours without clipping their contents; focus changes the same contour, not a second shadow. Alt+Up/Down navigates loaded conversation sections, including from an empty composer, while nonempty editors retain their keyboard behavior.
Remote subagent artifacts require `python3` on the allowed SSH destination's noninteractive PATH and use bounded, read-only multiplexed SSH channels with realpath confinement. Roster recovery scans the parent’s last 4MiB: older task results outside that window are not an exhaustive historical roster, and absence is not proof an agent never existed. Completion reads cap at 1MiB; transcripts page in 256KiB windows at complete-line byte boundaries. Missing artifacts return no content; unreadable, oversized-entry, and confinement failures are errors rather than fabricated completions. Ordinary interrupted turns recover despite dormant goal metadata; a pre-restart active autonomous goal remains paused.
Boundary regression checks: `lib/remote-subagent-history.test.mjs` invokes the embedded Python reader through a disposable local SSH shim and requires Python 3 plus a POSIX shell. `lib/visual-sanitize.test.mjs` runs real-browser sanitization when `VISUAL_SANITIZE_PLAYWRIGHT_DIR` points to a tooling directory containing `node_modules/playwright` and `node_modules/esbuild`; without it the browser case explicitly skips. No browser or DOM dependency is added to production.
Visuals preserve semantic layout tags and inert disabled mock buttons, with bounded width/color-scheme media queries, gradients, shadows, and overflow. Unsupported markup or CSS triggers a visible warning; scripts, handlers, forms, external resources, positioning escapes, and animation remain blocked. Push sends the explicit commit/integrate-upstream/push instruction; Feel asks “How do you feel about the work so far?” and Onwards sends “Onwards!”. Empty-composer quick replies remain visible while focused.
Quick replies use five icon-only 44px category triggers (Compass, Lightbulb, Flask, Heart, Play). Each opens an upward anchored popover containing the user's canned prompts; opening categories never replaces the row or resizes the composer. Choosing a prompt fills and focuses the editor without sending. The empty editor has two clear lines and a 12px gap before the category row.
Visuals render inline at full available width and natural height, without preview-card padding, height caps, source-copy controls, or version badges. The sole inline Full screen action opens an edge-to-edge, safe-area-aware dialog with a Close control; the original inline visual remains mounted when the viewer closes. Both views use the same sanitized Shadow DOM renderer.
...
User submissions append a server-generated `<system-reminder>` sidecar containing a fresh persisted Nook alias plus expression/caption age. This pragmatic path needs no native OMP patch: raw session files retain the sidecar, while Nook message rendering, previews, copy/edit text, and optimistic reconciliation strip its exact generated format. Reaction tools resolve aliases against exact message markers, persist the canonical target mapping, and accept the supplied current id without a list lookup. Slash commands and internal goal/recovery prompts are unchanged. Expression timestamps advance only on expression updates; older state has unknown age.
User emoji additions/removals on assistant messages notify that thread’s local or SSH agent through native `steer`, including the target id and a quoted excerpt. This does not abort active work; native steering wakes idle sessions, so feedback can produce a turn. The notice explicitly grants no permission to resume paused goals and requires no textual acknowledgment. A saved reaction whose delivery fails stays saved and shows a notification-error toast rather than pretending delivery succeeded.
...
Foreground return refreshes the selected local timeline even when the browser thinks the agent is idle, and replaces potentially half-open SSE connections. Local and remote views listen to visibility, online, pageshow, and window focus; refreshes preserve unsent composer drafts. Internal reaction feedback remains in native context but is excluded from displayed user bubbles and conversation anchors.
