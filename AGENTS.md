...
Remote subagent artifacts use bounded, read-only multiplexed SSH channels with realpath confinement. Roster recovery scans the parent’s last 4MiB (not exhaustive older history), completion reads cap at 1MiB, and transcripts page at complete-line byte boundaries. Ordinary interrupted turns recover despite dormant goal metadata; a pre-restart active autonomous goal remains paused.
